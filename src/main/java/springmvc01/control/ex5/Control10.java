...
public class Control10 {
  ...
  /* 뷰 컴포넌트를 경유하지 않고 바로 콘텐츠를 클라이언트에게 보내기I
   * => 문자열로 콘텐츠를 리턴하라!
   * => 문자열이 콘텐츠 임을 표시하기 위해 @ResponseBody 애노테이션을 붙여라!
   */
  @RequestMapping(path="/ex5/test03", produces="text/html;charser=UTF-8")// value나 path는 같다
  @ResponseBody
  public String test03() {
    return "<html><body><h1>test03</h1><p>ABC가나다123</p></body></html>";
  }
  
  /* 뷰 컴포넌트를 경유하지 않고 바로 콘텐츠를 클라이언트에게 보내기II
   * => ResponseEntity 객체를 리턴한다.
   * => DispatcherServlet(프론트 컨트롤러)은 리턴 값의 타입이 ResponseEntity이면,
   *    JSP 실행하지 않고 이 객체에 들어있는 콘텐츠와 헤더를 가지고
   *    클라이언트로 바로 출력한다.
   */
  ...
  public ResponseEntity<String> test04() {
    ...
    return new ResponseEntity(
        "<html><body><h1>test03</h1><p>ABC가나다123</p></body></html>",
        headers,
        HttpStatus.OK);
  }
